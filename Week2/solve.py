...
class Solution():
    ...
    def show_solution(self):
        ...
        for i, ((number_missionaries, number_cannnibals, side), node) in enumerate(zip(steps[1:], nodes[1:])):
            ...
            print(
                f"Step {i + 1}: Move {number_missionaries} missionaries  and {number_cannnibals} \
    ...
    def draw_edge(self, number_missionaries, number_cannnibals, side, depth_level):
        ...
        if Parent[(number_missionaries, number_cannnibals, side)] is not None:
            u = pydot.Node(str(Parent[(number_missionaries, number_cannnibals, side)] + (
                depth_level - 1, )), label=str(Parent[((number_missionaries, number_cannnibals, side))]))
            self.graph.add_node(u)

            v = pydot.Node(str((number_missionaries, number_cannnibals, side, depth_level)), label=str(
                (number_missionaries, number_cannnibals, side)))
            self.graph.add_node(v)

            edge = pydot.Edge(str(Parent[(number_missionaries, number_cannnibals, side)] + (depth_level - 1, )), str(
                (number_missionaries, number_cannnibals, side, depth_level)), dir='forward')
    ...
    def bfs(self):
        ...
        while q:
            ...
            u, v = self.draw_edge(number_missionaries,
                                  number_cannnibals, side, depth_level)
    ...
    def dfs(self, number_missionaries, number_cannnibals, side, depth_level):
        ...
        for x, y in self.options:
            next_m, next_c, next_s = number_missionaries + operation * \
                x, number_cannnibals + operation * y, int(not side)
